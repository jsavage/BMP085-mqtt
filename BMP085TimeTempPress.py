...
def mqtt_pubtemp(temperature):
#    temp=str(int(time.time()))+":"+str(temperature)
    temp="N:"+str(temperature)

    subprocess.Popen(['mosquitto_pub','-h','raspberrypi','-r','-t','rrd/srt','-m',temp],shell=False)
    subprocess.Popen(['mosquitto_pub','-h','mqtt.dioty.co','-u','consult.js@gmail.com','-P','f9c51072','-t','/consult.js@gmail.com/','-m',str(temperature)],shell=False)
def mqtt_pubpress(pressure):
    press=str(int(time.time()))+":"+str(pressure)
    subprocess.Popen(['mosquitto_pub','-h','raspberrypi','-r','-t','rrd/srp','-m',press],shell=False)
    subprocess.Popen(['mosquitto_pub','-h','mqtt.dioty.co','-u','consult.js@gmail.com','-P','f9c51072','-t','/consult.js@gmail.com/','-m',str(pressure)],shell=False)
...
print '{}:{}'.format(str(int(time.time())),str(temperature / 10.))
print '{}:{}'.format(str(int(time.time())),str(pressure / 100.))
mqtt_pubtemp(temperature/10.)
mqtt_pubpress(pressure/100.)
#time.sleep(10)
